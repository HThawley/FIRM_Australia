...
parser.add_argument('-v', default=1, type=int, required=False, help='whether to print optimisation steps to console')
...
def F(x):
    ...
    TDC = Transmission(S) if scenario>=21 else np.zeros((intervals, len(DCloss), x.shape[1])) # TDC: TDC(t, k), MW
    ...
    if scenario<=17:
        cost[-1,:], cost[-2,:] = [np.zeros((1, x.shape[1]))] * 2
...
def F_wrapper(x, callback=False):
    npop = x.shape[1]
    processes = cpu_count() if args.w == -1 else args.w
    ...
    if processes > 1:
        with Pool(processes=processes) as processPool:
            results = processPool.map(F, [x[:, n*vsize: min((n+1)*vsize, npop)] for n in r])
        results = np.concatenate(results)
    else:
        results = np.concatenate([F(x[:, n*vsize: min((n+1)*vsize, npop)]) for n in r])
