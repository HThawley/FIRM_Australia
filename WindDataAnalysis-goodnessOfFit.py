...
import warnings
from scipy.stats import exponweib, weibull_min, expon, genextreme, goodness_of_fit, weibull_max, pareto
...
def readFile(argTuple=None, path=None, stn=None, speedThreshold=None, goodnessOfFit=None):
    if argTuple is not None: 
    ...
    if goodnessOfFit is not None: 
        ...
        if goodnessOfFit == 1: 
            ...
            # poissonModelling(np.repeat(lengths['duration'], lengths['count']))
            paretoGoodness = paretoModelling(np.repeat(lengths['duration'], lengths['count']))
            return [stnNo, gustObs, durObs, startTime, meanRes, durationGoodness, paretoGoodness]
        else:
...
def statisticalAnalysis(df, lengths, speedThreshold, stnNo=None):
    ...
    duration = poissonModelling(np.repeat(lengths['duration'], lengths['count']), 
                                percentile = 0.75, plot=True, stnNo = stnNo)
...
def poissonModelling(dist, n_year=None):
    ...
    likelihoods = [dist.map(lambda val: poisson.pmf(val, mean)).prod(), 
                   dist.map(lambda val: nbinom.logpmf(val, r, p)).sum()]

    return likelihoods

def paretoModelling(dist): 
    
    try: gof = goodness_of_fit(pareto, dist, n_mc_samples=1000)[2]
    except ValueError: gof = pd.NA
    except TypeError: gof = pd.NA
...
#%%
    ...
    stns = set(stn['station no.'])

    os.chdir('BOM Wind Data')
    
    result=[]
    for folder in os.listdir():
        if 'MaxWindGust' in folder: continue

        activeDir = os.getcwd()
        os.chdir(folder)
        
        for file in tqdm(os.listdir(), desc=f'Reading {folder}'):
            if 'Data' in file:
                if file[11:17] in stns:
                    result.append(readFile(path=file, stn=stn, speedThreshold=speedThreshold, goodnessOfFit=1))
    
        os.chdir(activeDir)

    result = pd.DataFrame(result, columns = ['station no.', 'gustObs', 'stormCount', 'startTime', 
                                             'meanRes', 'durationGoodness', 'paretoGoodness'])
    
    # result[['poisson', 'nbinom']] = result['durationGoodness'].astype(str)\
    #     .str.replace('[', '').str.replace(']', '').str.split(',', expand=True)
    # result['poisson'], result['nbinom'] = result['poisson'].astype(float), result['nbinom'].astype(float)
    

    stn = stn.merge(result, on = 'station no.', how = 'outer')
    
    stn.to_csv('Results/statModels/paretoZones.csv')
    ...
    # stn = readAll(r'BOM Wind Data', speedThreshold, multiprocess=False, goodnessOfFit=exponweib)
